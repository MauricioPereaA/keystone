...
Distilled from the canonical `.claude/rules/` so Amazon Q reviews PRs against the
...
- **Work ID everywhere.** Every branch, commit subject, and PR title carries a Work
  ...
  Source: `conventions/conventions.json`, `.claude/rules/commit-and-pr.md`.
- **Single source of truth.** Convention patterns (Work ID / branch / commit / PR /
  ...
  Source: `.claude/rules/boundaries.md` §1, `magic-values.md`.
