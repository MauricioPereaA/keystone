...
function toRetentionDays(days) {
    const valid = Object.values(logs.RetentionDays).filter((v) => typeof v === "number");
    return valid.includes(days) ? days : logs.RetentionDays.ONE_MONTH;
}
/**
        ...
        NagSuppressions.addResourceSuppressions(this, [
            {
                ...
                reason: "The Lambda execution role and the API Gateway CloudWatch role use the minimal AWS-managed " +
                    "policies (basic execution / push-to-CloudWatch-logs); they are the least-privilege baseline.",
            },
            {
                id: "AwsSolutions-IAM5",
                reason: "Wildcards are confined to CloudWatch Logs stream ARNs under this service's log groups, " +
                    "scoped by the managed logging policies.",
            },
            {
                id: "AwsSolutions-APIG2",
                reason: "Request validation is defined per-method by the consuming service's API model; the golden " +
