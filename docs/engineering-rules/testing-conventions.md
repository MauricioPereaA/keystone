...
- The contract step is **generated/selected by the framework's workflow generator per language**, so "API-contract" means the same thing for every team (the `new-language` runbook adds the mapping — `docs/runbooks/new-language.md`).
